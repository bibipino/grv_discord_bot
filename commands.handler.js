...
export async function storeRefresh(interaction) {
    ...
    const shopHerbEmbedPrint = cityItems.filter((currentCityHerb) => {
        return currentCityHerb.quantity > 0 && currentCityHerb.type === "herbs"
    }).map(mapItem).join("\n")

    const shopPotionEmbedPrint = cityItems.filter((currentCityHerb) => {
        return currentCityHerb.quantity > 0 && currentCityHerb.type === "potions"
    }).map(mapItem).join("\n")

    const shopMineralEmbedPrint = cityItems.filter((currentCityHerb) => {
        return currentCityHerb.quantity > 0 && currentCityHerb.type === "minerals"
    }).map(mapItem).join("\n")

    const shopCompostEmbedPrint = cityItems.filter((currentCityHerb) => {
        return currentCityHerb.quantity > 0 && currentCityHerb.type === "composts"
    }).map(mapItem).join("\n")

    const shopPoisonEmbedPrint = cityItems.filter((currentCityHerb) => {
        return currentCityHerb.quantity > 0 && currentCityHerb.type === "poisons"
    }).map(mapItem).join("\n")

    const shopProjectileEmbedPrint = cityItems.filter((currentCityHerb) => {
        return currentCityHerb.quantity > 0 && currentCityHerb.type === "projectiles"
    }).map(mapItem).join("\n")

    const HerbEmbed = shopHerbEmbedPrint.length === 0 ? [] : [
        { name: 'Nome Erba | (Prezzo) | x Disponibili', value: shopHerbEmbedPrint},
        // { name: "\u200B", value: "\u200B"},
    ]
    const PotionEmbed = shopPotionEmbedPrint.length === 0 ? [] : [
        { name: 'Nome Pozione', value: shopPotionEmbedPrint},
        // { name: "\u200B", value: "\u200B"},
    ]
    const MineralEmbed = shopMineralEmbedPrint.length === 0 ? [] : [
        { name: 'Nome Minerale', value: shopMineralEmbedPrint},
        // { name: "\u200B", value: "\u200B"},
    ]
    const CompostEmbed = shopCompostEmbedPrint.length === 0 ? [] : [
        { name: 'Nome Composto', value: shopCompostEmbedPrint},
        // { name: "\u200B", value: "\u200B"},
    ]
    const PoisonEmbed = shopPoisonEmbedPrint.length === 0 ? [] : [
        { name: 'Nome Veleno', value: shopPoisonEmbedPrint},
        // { name: "\u200B", value: "\u200B"},
    ]
    const ProjectileEmbed = shopProjectileEmbedPrint.length === 0 ? [] : [
